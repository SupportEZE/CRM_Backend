...
export class PrivacyPolicyService {
  ...
  async create(req: Request, params: any): Promise<any> {
    try {
      const exist = await this.privacyPolicyModel.findOne({ org_id: req['user']['org_id'] }).exec();
      if (exist) {
        const updateObj = {
          ...req['updateObj'],
          ...params,
        };
        await this.privacyPolicyModel.updateOne({ _id: exist._id }, updateObj);
        ...
        const document = new this.privacyPolicyModel(saveObj);
        await document.save();
